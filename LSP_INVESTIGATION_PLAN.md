...
#### Phase 2-1: 最小動作プロトタイプ（3日以内）
**目標**: JSON-RPC中継の基本動作確認
...
2. **Day 2: パス変換実装**
   ...
   - `textDocument/didOpen` の変換確認
   - `textDocument/definition` レスポンス変換

3. **Day 3: 統合テスト**
   - go-exampleとgo-test-example両方で検証
   - ホバー、補完、定義ジャンプの動作確認
...
**最小基準（Phase 2-1完了時）**：
- ✅ go-exampleで全LSP機能動作
- ✅ go-test-exampleで全LSP機能動作
- ✅ 初期診断エラーなし

**本番基準（Phase 2-3完了時）**：
- ✅ 5つ以上の異なるGoプロジェクトで検証
- ✅ Python、TypeScriptでも基本動作確認
- ✅ 24時間連続動作テスト合格

### 即時アクション

1. **本日中**: Strategy B実装開始宣言
2. **明日まで**: jsonrpc.luaの基本実装完了
3. **3日以内**: 最小動作プロトタイプ完成

**重要**: gdマッピングは残すが、あくまで緊急回避策として位置づけ。Strategy B完成後は削除。
