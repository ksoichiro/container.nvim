  ...
  _G.vim = {
    ...
    fn = {
      ...
      executable = function(name)
        if name == 'docker' then
          return 1
        elseif name == 'gopls' or name == 'pyright-langserver' then
          return 1
        end
        return 0
      end,
      ...
      system = function(cmd)
        ...
        elseif cmd:match('docker.*inspect') then
          return 'running'
        elseif cmd:match('which.*gopls') then
          return '/usr/local/bin/gopls'
        elseif cmd:match('which.*pyright') then
          return '/usr/local/bin/pyright-langserver'
        end
        return 'success'
      end,
      ...
      has = function(feature)
        return feature == 'nvim-0.8' and 1 or 0
      end,
