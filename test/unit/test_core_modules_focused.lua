...
local function setup_minimal_vim_mock()
  _G.vim = {
    ...
    api = {
      ...
      nvim_buf_get_name = function() return '/workspace/test.go' end,
      nvim_buf_get_option = function(_, opt) return opt == 'filetype' and 'go' or nil end,
    ...
    fn = {
      expand = function(expr) return expr == '%:p' and '/workspace/test.go' or expr end,
      getcwd = function() return '/workspace' end,
      filereadable = function(path) return path:match('devcontainer%.json') and 1 or 0 end,
      readfile = function() return {'{\"name\": \"test\", \"image\": \"golang:1.21\"}'} end,
      system = function() vim.v.shell_error = 0; return 'Docker version 20.10.21' end,
    ...
    defer_fn = function(fn) if fn then fn() end end,
    notify = function() end,
    ...
    lsp = {
      ...
      handlers = {},
      protocol = {make_client_capabilities = function() return {} end},
    },
  }
end
...
local function setup_core_mocks()
  ...
  package.loaded['container.utils.notify'] = {
    notify = function() end, setup = function() end,
    container = function() end, progress = function() end,
  }
  ...
  package.loaded['container.config'] = {
    ...
    get = function() return {
      auto_open = 'off', log_level = 'info', container_runtime = 'docker',
      workspace = {auto_mount = true}, lsp = {auto_setup = false},
      ui = {use_telescope = false}, terminal = {integrated = true},
    } end,
    get_value = function(key)
      local vals = {lsp = {auto_setup = false}, terminal = {integrated = true}}
      return vals[key]
    end,
    show_config = function() return 'Configuration displayed' end,
  }

  -- Parser with minimal functions
  package.loaded['container.parser'] = {
    find_devcontainer_config = function() return nil end, -- Return nil to avoid complex parsing
    parse_devcontainer_json = function() return {name = 'test', image = 'golang:1.21'} end,
    validate = function() return {} end,
    find_and_parse = function() return nil end, -- Return nil to avoid complex parsing paths
    normalize_for_plugin = function(config) return config end,
    resolve_dynamic_ports = function(config) return config, nil end,
    validate_resolved_ports = function() return {} end,
    merge_with_plugin_config = function(config) return config end,
  }

  -- Terminal with no-op functions
  package.loaded['container.terminal'] = {
    setup = function() end,
    open_terminal = function() return 1 end,
  }

  -- LSP language registry
  package.loaded['container.lsp.language_registry'] = {
    get_by_filetype = function() return {server_name = 'gopls', filetype = 'go'} end,
    get_supported_languages = function() return {'go'} end,
  }
  ...
  package.loaded['container'] = {
    get_state = function() return {current_container = nil} end,
  }
end
...
run_test('Init module file operations', function()
  ...
  local reconnect_ok = pcall(function() init.reconnect() end)
