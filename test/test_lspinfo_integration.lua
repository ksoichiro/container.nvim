...
_G.vim = {
  fn = {
    ...
    sha256 = function(str) return "mockhash" end
  },
  api = {
...
local mock_log = {
  debug = function(...) end,
  info = function(msg, ...) print("[INFO] " .. string.format(msg, ...)) end,
  warn = function(msg, ...) print("[WARN] " .. string.format(msg, ...)) end,
  error = function(msg, ...) print("[ERROR] " .. string.format(msg, ...)) end
}
...
package.loaded['lspconfig.util'] = {
  find_git_ancestor = function() return "/test/project" end,
  path = {
    dirname = function(path) return path end
  }
}
