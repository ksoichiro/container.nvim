...
-- Telescope pickers for devcontainer.nvim
      ...
      attach_mappings = function(prompt_bufnr, map)
        actions.select_default:replace(function()
          ...
          if selection.type == 'container' then
            ...
            if container.running then
              ...
              require('devcontainer').attach(container.name)
            else
              -- Start stopped container
              require('devcontainer').start_container(container.name)
            end
          else
            -- Open project
            local project = selection.value
            require('devcontainer').open(project.path)
          end
        end)
        ...
        map('i', '<C-s>', function()
          ...
          if selection.type == 'container' and selection.value.running then
            require('devcontainer').stop_container(selection.value.name)
          end
        end)
        ...
        map('i', '<C-r>', function()
          ...
          actions.close(prompt_bufnr)
          if selection.type == 'container' then
            require('devcontainer').restart_container(selection.value.name)
          elseif selection.type == 'project' then
            require('devcontainer').rebuild(selection.value.path)
          end
        end)
...
function M.history(opts)
  opts = opts or {}
  local history = require('devcontainer.history').get_exec_history()
        ...
        actions.select_default:replace(function()
          ...
          vim.ui.input({
            ...
            if command and command ~= '' then
              require('devcontainer').exec(command)
            end
          end)
        end)
